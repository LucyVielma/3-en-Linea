...
if (typeof io === "undefined") {
  statusEl.textContent = "Error: no cargó Socket.IO. Recarga la página.";
  console.error("Socket.IO no cargó");
} else {
  const socket = io({
  ...
  // ✅ ESTE ERA EL QUE TE FALTABA (sin esto nunca “arranca” bien)
  socket.on('state', (s) => {
    state = s;
    render();
  });
  ...
  // Si falla conexión, no se queda pegado
  ...
  socket.on('disconnect', () => {
    // Si aún no estaba jugando, permite reintentar
    if (!mySymbol) {
      statusEl.textContent = 'Desconectado. Presiona “Entrar a jugar” de nuevo.';
      joinBtn.disabled = false;
    }
    setChatEnabled(!!mySymbol);
  });
  ...
  if (chatHandle) {
    ...
    chatHandle.addEventListener('pointerup', () => {
      dragging = false;
      keepChatInsideScreen();
    });
